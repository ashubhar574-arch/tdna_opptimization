        ...
        try:
            ...
            try:
                ...
                reference_df = reference_df.select(
                    ...
                    col("preferences").alias("preference_struct"),
                    F.to_json(col("preferences")).alias("preference_body_string"),
                    F.current_timestamp().alias("pref_ingestion_datetime"),
                ...
                membership_exploded = membership_result.select(
                    "profile_id",
                    "profile_version",
                    F.explode("memberships").alias("membership_item")
                ).select(
                    "profile_id",
                    "profile_version",
                    col("membership_item.member_id").alias("member_id")
                ).filter(col("member_id").isNotNull()).distinct()

                
                # Join preferences with membership data on member_id = ffp_number
                joined_preferences = membership_exploded.join(
                    broadcast(reference_df),
                    membership_exploded.member_id == reference_df.ffp_number,
                    "inner"
                ).select(
                    "profile_id",
                    "profile_version",
                ...
                if DeltaTable.isDeltaTable(spark, reference_prod_path):
                    ...
                    target_table.alias("target").merge(
                        ...
                        set={
                            ...
                            "pref_ingestion_datetime": "source.pref_ingestion_datetime",
                    ...
                    delta_write_df.write.format("delta").mode("overwrite").save(reference_prod_path)
                ...
                preferences_result = joined_preferences.groupBy("profile_id","profile_version").agg(
                    F.collect_list("preference_struct").alias("preferences")
                )
                
                # Ensure all profiles have preferences field (empty array if null)
                preferences_result = all_profiles.join(preferences_result, ["profile_id", "profile_version"], "left").withColumn(
                    "preferences",
